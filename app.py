...
def load_data_from_sheet():
    try:
        ...
        st.error(f"❌ 데이터 로딩 실패 (아직 시트가 없거나 비어있음): {e}")
        return {}
...
def init_session_state():
    ...
    if 'patient_db' not in st.session_state:
        ...
        if loaded_db:
            ...
            # [수정] PAGI 희석액 -> 인삼대사체(PAGI) 항암용 으로 변경
            items = [{"제품": "혼합 [Ex.P]", "용량": "150ml", "수량": 14, "타입": "혼합"}, {"제품": "혼합 [R.P]", "용량": "150ml", "수량": 14, "타입": "혼합"}, {"제품": "혼합 [Edf.P]", "용량": "150ml", "수량": 14, "타입": "혼합"}, {"제품": "혼합 [P.P]", "용량": "150ml", "수량": 14, "타입": "혼합"}, {"제품": "커드 시원한 것", "용량": "280ml", "수량": 14}, {"제품": "인삼대사체(PAGI) 항암용", "용량": "50ml", "수량": 14}]
            add_patient(db, "하혜숙", "격주 발송", "유방암", True, items)
            # 울산
            items = [{"제품": "장미꽃 대사체", "용량": "50ml", "수량": 7}, {"제품": "인삼대사체(PAGI) 항암용", "용량": "50ml", "수량": 4}, {"제품": "시원한 것", "용량": "280ml", "수량": 14}, {"제품": "계란 커드", "용량": "150g", "수량": 7}]
...
# 5. 메인 화면
st.title("🏥 엘랑비탈 ERP v.5.5 (Live DB)")
            ...
            with cols[i%2]:
                with st.container(border=True):
                    ...
                    for x in items:
                        ...
                        # [핵심 수정] 라벨에는 '항암용'이라는 글자를 제거하고 표시
                        display_prod = x['제품'].replace(" 항암용", "")
                        st.markdown(f"**{chk} {display_prod}** {x['수량']}개 ({x['용량']})")
                    st.markdown("---")
                    st.write("🏥 **엘랑비탈바이오**")

# Tab 2: 장연구원 (포장 리스트 - 여기서는 전체 이름 표시)
with t2:
